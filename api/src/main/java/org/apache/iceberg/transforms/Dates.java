...
enum Dates implements Transform<Integer, Integer> {
  ...
  @Override
  public UnboundPredicate<Integer> project(String fieldName, BoundSetPredicate<Integer> pred) {
    if (pred.op() == IN) {
  ...
  @Override
  public UnboundPredicate<Integer> projectStrict(String fieldName,
                                                 BoundSetPredicate<Integer> pred) {
    if (pred.op() == NOT_IN) {
      return predicate(pred.op(), fieldName,
          pred.literalSet().stream().map(v -> Literal.of(apply(v))).collect(Collectors.toSet()));
    }
